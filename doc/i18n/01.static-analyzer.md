...
```bash
# package.json
cross-env BLOG_POSTS_FOLDER=./posts BLOG_CONFIG_FILE=./src/config/blog.ts I18N_LOCALES_SCHEMA=./src/i18n/locales/schema.ts pnpm --filter @rtm/static-analyzer start
```
...
dist/main.js --posts-folder $BLOG_POSTS_FOLDER --blog-config-file $BLOG_CONFIG_FILE --i18n-locales-schema $I18N_LOCALES_SCHEMA
```
...
```bash
# packages/static-analyzer/package.json
dist/main.js --posts-folder $BLOG_POSTS_FOLDER --blog-config-file $BLOG_CONFIG_FILE --i18n-locales-schema $I18N_LOCALES_SCHEMA --skip-locales-infos # * ... Added --skip-locales-infos
```
