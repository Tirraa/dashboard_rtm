...
Basically, we just need to copy/paste `PostSchema` (or any another _Post_ type, based on `PostSchema`) and slightly adapt it:
...
```ts
// contentlayer.config.ts
export default makeSource({ contentDirPath, documentTypes: [PostSchema, PatchPost] });
```
...
```ts
// contentlayer.config.ts
export default makeSource({ contentDirPath, documentTypes: [PostSchema, PatchPost, DummyPost] });
```
