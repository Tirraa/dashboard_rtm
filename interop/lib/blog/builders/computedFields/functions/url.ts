...
export const buildBlogPostUrl = (post: PostBase): AppPath => {
  ...
  if (flattenedPathParts.length <= OPTIONAL_LOCALE_PART_INDEX + 1) return '/' + DEFAULT_LANGUAGE + ROUTES_ROOTS.BLOG + flattenedPath;

  const localePart = flattenedPathParts[OPTIONAL_LOCALE_PART_INDEX];
  flattenedPathParts.splice(OPTIONAL_LOCALE_PART_INDEX, 1);
  return '/' + localePart + ROUTES_ROOTS.BLOG + flattenedPathParts.join('/');
};
