...
import { AUTOGENERATED_CODE_COMMENT_STR, TS_MORPH_FORMATTER_SETTINGS, GENERATIONS_TARGET_FOLDER, PAGES_TITLES_CONST_STR } from '../../config';
...
export default async function generateI18nPagesTitles(
  ...
  const initializerWriterFunction: WriterFunction = Writers.assertion(
    JSON.stringify(pagesTitles, null, pretty ? 2 : undefined).replace(/""|''|``/g, '_'),
    'const'
  );
