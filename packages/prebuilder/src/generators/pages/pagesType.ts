...
const HARDCODED_FALLBACK_TYPE_FIELDS = new Set(['path', 'url', 'head', 'tail', 'pathWithoutHead', 'nestingLevelTwo']);
const HARDCODED_FALLBACK_TYPE_FIELDS_to_a = Array.from(HARDCODED_FALLBACK_TYPE_FIELDS).sort();

// {ToDo} Write tests to replace this safety check (using a fake page)
function generatePagesFromCodegenSchemaSafetyCheck(maybeObjEntry: unknown, fields: string[]) {
  if (maybeObjEntry === undefined) return;

  const allFields = Array.from(new Set(HARDCODED_FALLBACK_TYPE_FIELDS_to_a.concat(fields)));
  for (const field of allFields) {
    if (!HARDCODED_FALLBACK_TYPE_FIELDS.has(field) || !fields.includes(field)) {
      const expectedFields = fields.sort().join(', ');
      const gotFields = HARDCODED_FALLBACK_TYPE_FIELDS_to_a.join(', ');
      console.warn(formatMessage('hardcodedPagesTypeIsStale' satisfies VocabKey, { expectedFields, gotFields }));
      break;
    }
  }
}
...
function generatePagesFromCodegenSchema(pagesArchitecture: PagesMetadatas) {
  ...
  generatePagesFromCodegenSchemaSafetyCheck(maybeObjEntry, fields);

  const typeAcc = [];
  for (const field of fields) typeAcc.push(`"${field}"` + ':' + 'string');
  const type = typeAcc.join(';') + ';';
  return type;
}
